...
fn test_insert_and_get_account() {
    ...
    insert_account(conn, &acc).expect("insert should succeed");

    let found = get_account(conn, "a1")
...
fn test_account_crud() {
    ...
    insert_account(conn, &acc).expect("insert");

    let fetched = get_account_by_email(conn, "crud@example.com")
        .expect("get_by_email")
        .expect("should exist");
    ...
    update_account(conn, &updated).expect("update");

    let fetched = get_account(conn, "a2")
        .expect("get")
        .expect("should exist");
    ...
    insert_account(conn, &acc2).expect("insert second");

    let all = list_accounts(conn).expect("list");
    assert_eq!(all.len(), 2);

    set_active_account(conn, "a3").expect("set active");

    let a2 = get_account(conn, "a2").expect("get a2").unwrap();
    let a3 = get_account(conn, "a3").expect("get a3").unwrap();
    assert!(!a2.is_active);
    assert!(a3.is_active);

    delete_account(conn, "a2").expect("delete");
    assert!(get_account(conn, "a2").expect("get deleted").is_none());
    assert_eq!(list_accounts(conn).expect("list after delete").len(), 1);

    let result = delete_account(conn, "nonexistent");
...
fn test_sync_state_upsert() {
    ...
    upsert_sync_state(conn, &entry).expect("upsert");

    let fetched = get_sync_state(conn, "/home/test/file.txt")
        .expect("get")
        .expect("should exist");
    ...
    upsert_sync_state(conn, &updated).expect("upsert update");
    ...
    let all = list_sync_states(conn).expect("list");
    assert_eq!(all.len(), 1);

    delete_sync_state(conn, "/home/test/file.txt").expect("delete");
    assert!(get_sync_state(conn, "/home/test/file.txt")
        .expect("get after delete")
        .is_none());

    let result = delete_sync_state(conn, "/nonexistent");
    match result.unwrap_err() {
...
fn test_job_crud() {
    ...
    let fetched = get_job(conn, &job_id)
        .expect("get")
        .expect("should exist");
    ...
    update_job_state(conn, &job_id, JobState::Running, None).expect("update state");
    let fetched = get_job(conn, &job_id)
        .expect("get")
        .expect("should exist");
    assert_eq!(fetched.state, JobState::Running);

    update_job_state(conn, &job_id, JobState::Failed, Some("network error")).expect("update failed");
    let fetched = get_job(conn, &job_id)
        .expect("get")
        .expect("should exist");
    ...
    delete_job(conn, &job_id).expect("delete");
    assert!(get_job(conn, &job_id)
        .expect("get after delete")
        .is_none());

    let result = delete_job(conn, "nonexistent");
    match result.unwrap_err() {
...
fn test_list_by_state() {
    ...
    insert_job(conn, &j1).expect("insert j1");
    insert_job(conn, &j2).expect("insert j2");
    insert_job(conn, &j3).expect("insert j3");
    insert_job(conn, &j4).expect("insert j4");

    let queued = list_jobs(conn, Some(JobState::Queued)).expect("list queued");
    assert_eq!(queued.len(), 2);
    assert_eq!(queued[0].priority, 20);
    assert_eq!(queued[1].priority, 10);

    let all = list_jobs(conn, None).expect("list all");
    assert_eq!(all.len(), 4);

    clear_completed_jobs(conn).expect("clear completed");
    let all = list_jobs(conn, None).expect("list after clear");
    assert_eq!(all.len(), 3);

    let completed = list_jobs(conn, Some(JobState::Completed)).expect("list completed");
